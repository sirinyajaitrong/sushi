...
<div class="container"><br />
    <div class="row">	
    <div class="col-md-2">
    <label class="label label-warning"  style="font-size: 18px !important;" ><b>จัดการขายสินค้า</b></label>
    </div>
...
<div class="container-full">    
        <div id="loginbox" style="margin-top:5px;" class="mainbox col-md-12 col-md-offset-0 col-sm-8 col-sm-offset-2">                    
            <div class="panel panel-info f16" >
                    ...
                    <p>
                    <div class="table-responsive center" style="width: 98%;margin-left:1%; color: #101010;">
                    <table class="table table-bordered table-hover f16 center" id="myTable">
                        <thead>
                            ...
                            <?php } ?>   
                            <?php if($customer_id == "001" && $_SESSION["status"] != "2") { ?>
                                <th class="text-center">จัดการ</th>
                            <?php } else if(($customer_id != "001" && $_SESSION["status"] != "2")||($customer_id != "001" && $_SESSION["status"] == "2")) { ?>
                                <th class="text-center">จัดการ</th>
                            <?php } ?>  
                            </tr>
                            ...
                            if ($rows_sell != false) {
                                ...
                                foreach ($rows_sell as $row_sell) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center" ><?= $row_sell["products_name"] ?></td>
                                        ...
                                        <td class="text-center"><?= $row["price"] ?> บาท</td> -->
                                        <td class="text-right" ><?= number_format($row_sell["sell_quantity"]) ?> แพ็ค</td>
                                        <td class="text-center" style="width: 200px;"><?= DateThaiTime($row_sell["date"]) ?></td>
                                        <td class="text-right" ><?= number_format($row_sell["sell_sumprice"],2) ?> </td>
                                        <td class="text-right" style="width: 120px;"><?= number_format($row_sell["sell_total"],2) ?> </td>
                                    <?php if($customer_id != "001") { ?>   
                                        <td class="text-center" style="width: 70px;" > <img style="border-radius: 40%;" onclick="showPic('./upload_img/<?= $row_sell['slip'] ?>')" src="./upload_img/<?= $row_sell["slip"] ?>" width="30px;" height="30px" alt=""></td>
                                    <?php } ?> 
                                        <td class="text-right" style="width: 120px;"><?= number_format($row_sell["pay"],2) ?> </td>
                                    <?php if($customer_id != "001") { ?> 
                                        <!-- <td class="text-right" style="width: 120px;"><?= number_format($row_sell["sell_sumprice"]-$row_sell["pay"],2) ?> </td> -->
                                    <?php } ?>  
                                        
                                    <?php if($customer_id != "001") { ?>
                                        <td class="text-center" style="width: 200;"><?php if(!empty($row_sell["date_pay"])){ echo DateThaiTime($row_sell["date_pay"]); } ?></td>
                                        <td class="text-center" style="width: 100px;"><?= $row_sell["delivery_status_name"] ?></td>
                                    <?php } ?>
                                    <?php if($customer_id == "001" && $_SESSION["status"] != "2" && $row_sell["pay"] == 0) { ?>
                                        <td class="text-center f14" style="width: 20%;">
                                            <?php if($row_sell["pay"] != $row_sell["sell_total"]){ ?> 
                                                <span onclick="AddPay('<?= $row_sell["sell_id"] ?>', '<?= $row_sell["products_name"] ?>', '<?= $row_sell["sell_total"]-$row_sell["pay"] ?>')" class="">
                                                    <i class="fa fa-paypal fa-lg" aria-hidden="true" style="color: #04bb1c"></i>
                                                </span>
                                            <?php } else { ?>
                                            <?php } ?>
                                            <span href="#" data-href="conSell.php?action=delete&sell_id=<?= $row_sell["sell_id"] ?>&products_id=<?= $row_sell["products_id"] ?>&sell_quantity=<?= $row_sell["sell_quantity"] ?>&customer_id=<?= $customer_id ?>" data-toggle="modal" data-target="#confirm-delete" class="">
                                                <i class="fa fa-window-close fa-lg" aria-hidden="true" style="color: #f32b2b"></i>
                                            </span>  
                                        </td>    
                                    <?php } else if(($customer_id != "001" && $_SESSION["status"] != "2")||($customer_id != "001" && $_SESSION["status"] == "2")) { ?>
                                        <td class="text-center f14" style="">
                                            <?php if($customer_id != "001") { 
                                                    if($row_sell["pay"] != $row_sell["sell_total"]){ ?> 
                                                        <span onclick="AddPay('<?= $row_sell["sell_id"] ?>', '<?= $row_sell["products_name"] ?>', '<?= $row_sell["sell_total"]-$row_sell["pay"] ?>')" class="">
                                                            <i class="fa fa-paypal fa-lg" aria-hidden="true" style="color: #04bb1c"></i>
                                                        </span>
                                                    <?php } ?>
                                            ...
                                            <?php if($_SESSION["status"] != "2" && $row_sell["delivery_status_id"] != "1" && $row_sell["pay"] == 0){ ?>
                                              <span href="#" data-href="conSell.php?action=delete_r&sell_id=<?= $row_sell["sell_id"] ?>&products_id=<?= $row_sell["products_id"] ?>&sell_quantity=<?= $row_sell["sell_quantity"] ?>&customer_id=<?= $customer_id ?> " data-toggle="modal" data-target="#confirm-delete" class="">
                                                <i class="fa fa-window-close fa-lg" aria-hidden="true" style="color: #f32b2b"></i>
                                            </span>  
                                            <?php } ?>
                                        </td>    
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">	
            ...
            <div class="col-md-4 f16">
                <label>ยอดชำระ</label>
                <input type="number" id="c_pay" name="c_pay"  class="form-control text-right" min="1" value="1" readonly  required="" />
            </div>
...
<!-- Modal -->
<div class="modal fade" id="paySlipModal" tabindex="-1" role="dialog" aria-labelledby="paySlipModal" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="paySlipModalLabel" style="text-align: center; font-weight: bold;"></h5>
        <!-- <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button> -->
      </div>
...
<script>
    ...
    function AddPaySlip(sell_id, products_name){
        //alert(sell_id);
        var e = document.getElementById("customer_id");
        var customer_id = e.options[e.selectedIndex].value;
        $('#sell_id_s').val(sell_id);
        $('#customer_id_s').val(customer_id);
        $("#paySlipModalLabel").text(products_name);
    ...
    //
    function ActionPaymentSlip(){
        var e = document.getElementById("customer_id");
        var customer_id = e.options[e.selectedIndex].value;
        var sell_id = $('#sell_id').val();
        var fileVal=document.getElementById("slip");
        //alert(fileVal.value);
