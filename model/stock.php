...
//stock_id	store_id	products_id	date	stock_quantity
    ...
    public function insert($data) {
        ...
        $this->sql = "SELECT * FROM stock WHERE store_id = {$data["store_id"]} AND products_id = {$data["products_id"]} 
        ...
        if ($num > 0) {
            ...
            while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
                $this->sql = "UPDATE stock SET stock_quantity = stock_quantity + {$data["stock_quantity"]}   
    ...
    public function delete($condition) {
        ...
        $this->sql_p = "DELETE FROM stock WHERE {$condition}";
    ...
    public function read($condition = "1=1 ") {
        ...
        $this->sql = "SELECT * FROM stock s 
          ...
          LEFT OUTER JOIN store sto ON s.store_id = sto.store_id   
